...
def render_ringkasan_eksekutif():
    ...
    df_parfum_re = la.hanya_kategori(df_re, "PARFUM") if TAMPILKAN_PARFUM else df_re.iloc[0:0]
    ...
    parfum_stok_re = (
        lp.apply_filters(df_persediaan, kategori="PARFUM", filter_luna=None)
        if (df_persediaan is not None and TAMPILKAN_PARFUM) else pd.DataFrame()
    )

    opk_re = la.omzet_per_kelompok(df_aks_re, df_parfum_re, keyword_brand="LUNA")
    if not TAMPILKAN_PARFUM:
        opk_re = opk_re[opk_re["Kelompok"] != "Parfum"].reset_index(drop=True)
    ...
    n_kartu = 3 if TAMPILKAN_PARFUM else 2
    c1, c2, c3 = st.columns(3) if TAMPILKAN_PARFUM else (*st.columns(2), None)
    if not opk_re.empty:
        for col, (_, row) in zip([c1, c2, c3][:n_kartu], opk_re.iterrows()):
            with col:
    ...
    if nilai_stok_kelompok:
        s1, s2, s3 = st.columns(3) if TAMPILKAN_PARFUM else (*st.columns(2), None)
        kelompok_list = ["Aksesoris LUNA", "Aksesoris Selain LUNA", "Parfum"] if TAMPILKAN_PARFUM else ["Aksesoris LUNA", "Aksesoris Selain LUNA"]
        for col, kelompok in zip([s1, s2, s3][:n_kartu], kelompok_list):
            with col:
...
def render_dashboard_scoreboard_aksesoris():
    ...
    if scoreboard.empty:
        ...
        if produk_scoreboard.empty:
            ...
            top_produk["Harga Modal / Pcs"] = top_produk["Harga Modal / Pcs"].map(la.format_rupiah_id)
...
def render_aksesoris_tab():
    ...
    # -----------------------------------------------------------------
    st.header("💰 Total HPP Aksesoris LUNA (dari Faktur Penjualan)")
    st.caption(
    ...
    if df is None:
        ...
        gunakan_filter_periode_hpp = st.checkbox(
            "Batasi ke periode tertentu (kosongkan untuk SELURUH data)", value=False, key="ak_hpp_gunakan_filter",
        )

        if not gunakan_filter_periode_hpp:
            tgl_mulai_hpp = df["TGL FAKTUR"].min()
            tgl_selesai_hpp = df["TGL FAKTUR"].max()
            ...
            mode_periode_hpp = st.radio(
                ...
                horizontal=True, key="ak_hpp_mode_periode",
            ...
            if mode_periode_hpp == "Periode Samurai (Kuartalan)":
                periode_pilihan_hpp = st.selectbox("Pilih Periode Samurai", periode_samurai_hpp_opsi, key="ak_hpp_periode_samurai")
                ...
                with ht1:
                    tgl_mulai_hpp = pd.Timestamp(st.date_input("Mulai Program", value=pd.Timestamp("2026-08-20"), key="ak_hpp_mulai"))
                with ht2:
                    durasi_hpp = st.slider("Durasi Program (bulan)", min_value=1, max_value=12, value=3, key="ak_hpp_durasi")
        ...
        df_hpp = df[
            (df["TGL FAKTUR"] >= tgl_mulai_hpp) & (df["TGL FAKTUR"] <= tgl_selesai_hpp)
        ]

        hpp = la.total_hpp_brand(df_hpp, keyword="LUNA", keyword_kecuali=None)
    ...
    if df_semua_kategori is not None:
        st.header("📋 Kepatuhan Bundling Aksesoris pada Transaksi Service")
        ...
        mode_periode_bund = st.radio(
            "Jenis Periode", ["Periode Samurai (Kuartalan)", "Program Custom (1–12 Bulan)"],
            horizontal=True, key="ak_bundling_mode_periode",
            help="Periode Samurai = kuartalan tetap (Jul 2026–Des 2027). Program Custom = atur sendiri tanggal mulai & durasi (1–12 bulan).",
        )
        ...
        if mode_periode_bund == "Periode Samurai (Kuartalan)":
            periode_pilihan_bund = st.selectbox("Pilih Periode Samurai", periode_samurai_bund_opsi, key="ak_bundling_periode_samurai")
            ...
            with bt1:
                tgl_mulai_bund = pd.Timestamp(st.date_input("Mulai Program", value=pd.Timestamp("2026-08-20"), key="ak_bundling_mulai"))
            with bt2:
                durasi_bund = st.slider("Durasi Program (bulan)", min_value=1, max_value=12, value=3, key="ak_bundling_durasi")
        ...
        df_semua_kategori_bund = df_semua_kategori[(df_semua_kategori["TGL FAKTUR"] >= tgl_mulai_bund) & (df_semua_kategori["TGL FAKTUR"] <= tgl_selesai_bund)]
        df_bund = df[(df["TGL FAKTUR"] >= tgl_mulai_bund) & (df["TGL FAKTUR"] <= tgl_selesai_bund)] if df is not None else df

        bund_info, bund_detail = la.analisa_bundling_brand(df_bund, df_semua_kategori_bund, keyword="LUNA")
        if bund_info["jumlah_nota_service"]:
            ...
            bund_cabang = la.analisa_bundling_per_cabang(df_semua_kategori_bund, keyword="LUNA")
            if bund_cabang.empty:
                ...
                st.download_button(
                    ...
                    "porsi_tanpa_bundling_per_cabang.csv", "text/csv", key="ak_dl_bundling_cabang",
                )
            ...
            with st.expander(f"🔍 Lihat Rincian Nomor Nota — {la.format_int_id(bund_info['jumlah_service_tanpa_aksesoris'])} Nota Tanpa Bundling", expanded=False):
                ...
                cabang_pilihan_bund = st.selectbox("Filter Cabang", cabang_opsi_bund, key="ak_bundling_cabang_filter")
                ...
                st.download_button(
                    ...
                    "rincian_nota_tanpa_bundling.csv", "text/csv", key="ak_dl_bundling_detail",
                )
        else:
...
def render_omzet_ldm_tab():
    ...
    st.caption(
        ...
        "bundling di transaksi Service (tetap dihitung sebagai penjualan aksesoris yang sah)."
    )
